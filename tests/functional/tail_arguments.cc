...
#include "../../etude/functional/tail_arguments.hpp"
...
{
  typedef decltype( etude::group( std::declval<Args>()... ) ) group_t;
      
  STATIC_ASSERT((
    std::is_same< group_t,
      decltype(
        etude::tail_arguments( std::declval<T>(), std::declval<Args>()... )
      )
    >::value
  ));
  ...
  STATIC_ASSERT((
    std::is_same<
      decltype(
        etude::pack( std::declval<group_t>() )
      ),
      decltype(
        etude::pack(
          etude::tail_arguments( etude::unpack( std::declval<tuple_type>() ) )
        )
      )
    >::value
  ));
...
int test_main( int, char** )
{
  ...
  auto p = etude::pack( etude::tail_arguments( 1, const_lvalue, etude::unpack(t) ) );
