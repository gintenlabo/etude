...
//
#include "../../etude/functional/less_equal.hpp"
...
{
  // 型特性
  check_basic_traits<etude::less_equal<T, U>>();
  check_basic_traits<etude::less_equal<U, T>>();
  
  // typedef
  STATIC_ASSERT((
    std::is_same<
      typename etude::less_equal<T, U>::first_argument_type, T
    >::value
  ));
  STATIC_ASSERT((
    std::is_same<
      typename etude::less_equal<T, U>::second_argument_type, U
    >::value
  ));
  STATIC_ASSERT((
    std::is_same<
      typename etude::less_equal<U, T>::first_argument_type, U
    >::value
  ));
  STATIC_ASSERT((
    std::is_same<
      typename etude::less_equal<U, T>::second_argument_type, T
    >::value
  ));
  
  // 実際に比較する
  etude::less_equal<>     const comp0 = {};
  etude::less_equal<T, U> const comp1 = {};
  etude::less_equal<U, T> const comp2 = {};
...
void check_not_comparable( T const&, U const& )
{
  ...
  STATIC_ASSERT((
    !etude::is_simply_callable<etude::less_equal<>, signature1>::value
  ));
  STATIC_ASSERT((
    !etude::is_simply_callable<etude::less_equal<T, U>, signature1>::value
  ));
  
  STATIC_ASSERT((
    !etude::is_simply_callable<etude::less_equal<>, signature2>::value
  ));
  STATIC_ASSERT((
    !etude::is_simply_callable<etude::less_equal<U, T>, signature2>::value
  ));
...
{
  check_basic_traits<etude::less_equal<>>();
