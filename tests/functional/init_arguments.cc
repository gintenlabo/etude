...
#include "../../etude/functional/init_arguments.hpp"
...
inline void check( Args&&... args )
{
  ...
  STATIC_ASSERT((
    std::is_same< pack_t,
      decltype(
        etude::pack(
          etude::init_arguments( std::declval<Args>()... )
        )
      )
    >::value
  ));
  ...
  STATIC_ASSERT((
    std::is_same< pack_t,
      decltype(
        etude::pack(
          etude::init_arguments( etude::unpack( std::declval<tuple_type>() ) )
        )
      )
    >::value
  ));
  ...
  auto p = etude::pack( etude::init_arguments( std::forward<Args>(args)... ) );
  (void)t, (void)p;
}
...
int test_main( int, char** )
{
  ...
  auto p = etude::pack( etude::init_arguments( const_lvalue, etude::unpack(t) ) );
