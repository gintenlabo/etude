//
//  identity_greater_equal:
//    アドレスを <= で比較する関数オブジェクト
// 
//    etude::identity_greater_equal<T, U> は、 T cv&, U cv& 型の引数 lhs, rhs に対し
//    !etude::identity_before( lhs, rhs ) を呼び出す、
...
namespace etude {
  ...
  struct identity_greater_equal_ {};
  
  template<class T, class U>
  struct identity_greater_equal_< T, U,
  ...
  struct identity_greater_equal
    : identity_greater_equal_<T const volatile&, U const volatile&> {};
  ...
  struct identity_greater_equal<>
  {
    ...
    // etude::identity_greater_equal<T&, U&> に転送
    template< class T, class U,
      class = typename std::enable_if<
        etude::is_simply_callable<
          etude::identity_greater_equal<T&, U&>, bool ( T&, U& )
        >::value
      >::type
    >
    bool operator()( T& lhs, U& rhs ) const {
      return etude::identity_greater_equal<T&, U&>()( lhs, rhs );
    }
